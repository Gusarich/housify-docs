...
This example defines a house named `ExampleHouse` with a global stat `counter` initialized to `0`. The `JOIN` event handler increments the `counter` stat by `1` every time a player joins the plot. You can then use the stat in other parts of code or even manually as `%stat.global/counter%` placeholder.
